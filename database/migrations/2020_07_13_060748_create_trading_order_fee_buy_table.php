...
class CreateTradingOrderFeeBuyTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('trading_order_fee_buy', function (Blueprint $table) {
            $table->id();
            ...
            $table->datetime('start_at')->comment('Thời gian bắt đầu áp dụng');
            $table->datetime('end_at')->nullable()->comment('Thời gian kết thúc áp dụng');
            $table->double('min_amount', 10, 2)->comment('Số tiền giao dịch tối thiểu');
            ...
            $table->integer('requested_by')->nullable()->comment('Người gửi yêu cầu');
    ...
    {
        Schema::dropIfExists('trading_order_fee_buy');
    }
}
