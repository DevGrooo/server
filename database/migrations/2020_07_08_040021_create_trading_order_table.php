...
class CreateTradingOrderTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        Schema::create('trading_orders', function (Blueprint $table) {
            ...
            $table->integer('file_import_line_id')->index()->nullable()->comment('File import line');
            ...
            $table->integer('investor_sip_id')->index()->nullable()->comment('Lệnh SIP của NĐT');
            ...
            $table->integer('send_account_system_id')->index()->nullable()->comment('Tài khoản dùng khi bán');
            $table->integer('send_fund_certificate_id')->index()->nullable()->comment('Quỹ bán');
            ...
            $table->integer('send_investor_fund_product_id')->index()->nullable()->comment('Tài khoản sản phẩm quỹ người bán');
            ...
            $table->integer('receive_account_system_id')->index()->nullable()->comment('Tài khoản nhận khi mua');
            $table->integer('receive_fund_certificate_id')->index()->nullable()->comment('Quỹ mua');
            ...
            $table->integer('transaction_system_id')->index()->nullable()->comment('Giao dịch VND trên hệ thống');
            $table->integer('transaction_fund_id')->index()->nullable()->comment('Giao dịch quỹ');
            $table->datetime('created_date')->nullable()->comment('Ngày đăng ký giao dịch');
            $table->integer('status')->comment('Trạng thái lệnh FMC(1:Mới tạo,2:Đã xác nhận,3:Đã hủy,4:Hoàn thành)');
            $table->string('reason_cancel')->nullable()->comment('Lý do hủy');
            $table->integer('vsd_status')->comment('Trạng thái lệnh VSD(1:Chưa import VSD,2: Đã import VSD, 3:Không khớp,4:Khớp)');
